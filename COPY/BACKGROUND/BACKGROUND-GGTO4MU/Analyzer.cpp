...
//TCanvas *c1 = new TCanvas("c1","c1");
	...
	RooRealVar a("a","a",0.1,-6.1,6.7) ;
	RooRealVar b("b","b",38.0) ;
 ...
 /*g.fitTo(wdata, Range(105,140));
   ...
   c1->SaveAs("test-background-gg4.pdf");
	...
	RooAbsReal* nll = g.createNLL(wdata, NumCPU(6));
	RooMinimizer(*nll).migrad();
	 RooPlot* frame1 = a.frame(Bins(100),Range(-6.1,6.7),Title("LL and profileLL in frac")) ;
   nll->plotOn(frame1,ShiftToZero()) ;
	RooAbsReal* pll_frac = nll->createProfile(a) ;
...
canv->SaveAs("maxlikelihood-ggto4m-a.pdf");
   delete pll_frac ;
   delete nll ;
