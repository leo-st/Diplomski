...
TCanvas *c1 = new TCanvas("c1","c1");
	...
	RooRealVar KFactor_EW_qqZZ("KFactor_EW_qqZZ","KFactor_EW_qqZZ",0.8,1.1) ;
   ...
   RooFormulaVar wFunc("gen","(LumiNumber * 1000 * xsec * KFactor_EW_qqZZ * overallEventWeight * L1prefiringWeight ) / genHEPMCweight",RooArgSet(LumiNumber,xsec, KFactor_EW_qqZZ, overallEventWeight,L1prefiringWeight,genHEPMCweight)) ;
...
suma = suma+((137.0 * 1000 * 0.00158549 * KFactor_EW_qqZZ * overallEventWeight ) / 594858.56);
//cout<<jentry<<" : "<<L1prefiringWeight<<endl;
	}
	if(ZZMass>= 105.0 && ZZMass<=140.0){
	//cout<<"jentry: "<<jentry<<"	ggH_NNLOPS_weight: "<<ggH_NNLOPS_weight<<endl;
if(KFactor_EW_qqZZ<min){
min=KFactor_EW_qqZZ;}
if(KFactor_EW_qqZZ>max){
max=KFactor_EW_qqZZ;}
	}
      if(ZZMass>= 105.0 && ZZMass<=140.0){
	cout<<"jentry: "<<jentry<<"	"<<KFactor_EW_qqZZ<<endl;
	}	
   }
